...
function CookieBanner() {
  ...
  return on ? (
    <div className="fixed inset-x-0 bottom-[5rem] z-10 flex flex-col bg-white p-[2rem]">
      ...
      <div className="mb-[1.6rem] flex justify-end gap-4">
        <button
          className="bg-white py-[12px] px-[60px] text-14 tracking-[2px] text-blue hover:bg-ghost"
          onClick={() => update(false)}
        >
          Deny
        </button>
        <button className="py-[12px] px-[60px] text-14 tracking-[2px]" onClick={() => update(true)}>
          Allow
        </button>
      </div>
    </div>
